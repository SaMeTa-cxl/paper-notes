...
def create_paper_markdown(sheet_name, df):
    """创建论文markdown（包含完整信息）"""
    md_lines = []
...
def create_readme(sheets_data):
    ...
    for category, sheet_names in categories.items():
        ...
        for sheet_name in sheet_names:
            if sheet_name in sheets_data:
                ...
                md_lines.append(f"- [{sheet_name}](paper-list/{filename}.md) ({count} 篇论文)\n")
        md_lines.append("\n")
    ...
    # 使用说明
    md_lines.append("\n## 使用说明\n\n")
    md_lines.append("1. 在 `paper-list/` 目录中查看各主题的论文笔记\n")
    md_lines.append("2. 修改Excel后运行脚本自动更新: `python scripts/convert_papers_to_markdown.py`\n")
...
def create_summary_md(sheets_data):
    ...
    for category, sheet_names in categories.items():
        ...
        for sheet_name in sheet_names:
            if sheet_name in sheets_data:
                filename = sheet_to_filename(sheet_name)
                md_lines.append(f"  - [{sheet_name}](paper-list/{filename}.md)\n")
        md_lines.append("\n")
...
def main():
    ...
    if not repo_path.exists():
        ...
        # 确保子目录存在
        (repo_path / 'paper-list').mkdir(exist_ok=True)
    ...
    for sheet_name, df in sheets_data.items():
        ...
        # 创建markdown文件
        paper_content = create_paper_markdown(sheet_name, df)
        (repo_path / 'paper-list' / f'{filename}.md').write_text(paper_content, encoding='utf-8')
    ...
    print("  git add .")
    print("  git commit -m 'Update paper notes'")
