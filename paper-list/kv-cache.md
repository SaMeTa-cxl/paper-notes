...
### CacheSlide(SJTU,FAST26) \[[Paper](https://www.usenix.org/conference/fast26/presentation/liu-yang)] \[[Code](https://github.com/SJTU-Storage-Lab/CacheSlide)] 


**核心思想**
* 分为三个模块：CCPE、WCA和SLIDE
* CCPE是利用位置不敏感的CoPE编码来解决复用文段位置漂移导致无法直接使用的问题，
它让复用段内部，以及复用段之间的attention可以直接复用，之前的工作无法做到。
* WCA用于修正复用段对动态段的attention，大致方法就是找偏差大的然后进行fuse。
* SLIDE是一个优化SSD读写的KV cache管理器，在vllm的page管理基础上增加了脏页的
管理，即是否有需要更新的KV，尽量offload干净的页来避免SSD写入，尽量offload修改多的
页来实现coalesced writing，从而减少写放大效应。

**思考与疑问**
* prompt假设的结构是预设的固定模板，这是否合理
* 离线采样的方式是否合理，每来一个新的模板，都需要重新采样
* WCA算法有点诡异，怀疑是写错了，为什么是相似度小于阈值就踢出纠正集合
* SLIDE的实现有点理解不了，原理能懂，但是想不到实现方式，不知道怎么样实现减少写放大效应
* 与CacheBlend、MEPIC这些方法的本质区别到底是什么，其实都是在prefill阶段找到一些token来修正的，
目前感觉就是本文的修正方式会更灵活一点，更细粒度一点
* fuse的方法能不能改进
* dirty-aware的思路能不能迁移到别的场景，比如说kv cache压缩、推测性解码之类的